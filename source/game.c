...
static void blink_star( void );
static void disp_title( void );
static void draw_meteo_field( void );
static void disp_over( void );
static void set_new_line( int pos );
static void init_meteo_field( void );
static void draw_meteo_field( void );
static void draw_map_meteo( int offset, u16 chr );
static void create_bg_map( void );
static void scroll_meteo_field( void );
static void init_shot( void );
static void launch_shot( void );
static void move_shot( void );
static bool is_hit_shot( int x, int y );
static void level_up( void );
static void check_line( void );
static void set_flash_line( int line );
static void flash_lines( void );
static void delete_line( int line );
static void draw_flash_line( int line );
static void init_flash_line( void );
static void set_aim( void );
static void init_aim( void );
static void add_score( int line );
static void add_line( void );
static void update_scores( void );
static void update_levels( void );
static void update_lines( void );
static void disp_num( int x, int y, u32 num );
static void disp_num_title( int x, int y, u32 num );
static void check_over( void );
static void pause( void );
static void init_comet( void );
static void move_comet( void );
static void appeare_comet( void );
static bool is_hit_ship( int y );
static void check_ship_status( void );
static void set_meteo_x3( int x, int y );
static void disp_ready( void );
static void set_ready( void );
static void check_over_wait( void );
static bool is_flash_line( void );
static void blink_sprite(void);
static void erase_aim(void);
...
appeare_comet( void )
{
  ...
  if( RND( 0, APPEAR_COMET_RND) || game_state.comet_num >= MAX_COMET ) return;
  ...
  set_sprite_tile( SPRITE_COMET + game_state.comet_num, TILE_CHR_COMET + comet[game_state.comet_num].kind * 8 );
...
move_comet( void )
{
  ...
  if( ! game_state.comet_num  ) return;
          ...
          if( is_hit_ship( y ) && ! ship.status )
            {
              ...
              // 自機絵柄
              set_sprite_tile( SPRITE_SHIP, TILE_CHR_SHIP_COMET1 + comet[ curr ].kind * 8 );
            }
...
level_up( void )
{
  if( --level[ game_state.mode ].up_count || level[ game_state.mode ].lv >= MAX_LEVEL ) return;
...
move_ship( void )
{
  ...
  if( (key & KEY_UP) && ship.y > 0 + SHIP_MOVE_STEP && ship.status != SHIP_DONTMOVE )
    {
      ship.y -= SHIP_MOVE_STEP;
      game_state.shot_interval = 1;
    }
  else if( (key & KEY_DOWN) && ship.y < SCREEN_HEIGHT - SHIP_MOVE_STEP && ship.status != SHIP_DONTMOVE )
    {
      ship.y += SHIP_MOVE_STEP;
      game_state.shot_interval = 1;
    }
...
launch_shot( void )
{
  ...
  if(  game_state.shot_num < MAX_SHOT && (key & KEY_A))
    {
      // スプライト
      move_sprite( game_state.shot_num + SPRITE_SHOT, ship.x + DEF_SHOT_X, ship.y );
...
set_meteo_x3( int x, int y )
{
  ...
  if( ship.status != SHIP_SHOTX3 ) return;

  for( i = 0; i < 2; i++ )
    {
      if( ! (FIELD_WIDTH + x - i) ) break;
...
flash_lines( void )
{
  ...
  for( i = 0; i < FIELD_WIDTH; i++ )
    {
      if( ! flash_line[ i ].wait_rel ) continue;
...
is_flash_line( void )
{
  ...
  for( i = 0; i < FIELD_HEIGHT; i++ )
    {
      if( flash_line[ i++ ].count ) return true;
    }
...
scroll_meteo_field( void )
{
  ...
  if( --game_state.scroll_speed ) return;
  game_state.scroll_speed = game_state.scroll_speed_rel;

  if( ++game_state.scroll_offset > LIMIT_SCROLL_OFFSET ) game_state.scroll_offset = 0;

  if( game_state.scroll_offset ) return;
...
check_over( void )
{
  ...
  if( flg != 0 && flg != (CHR_FLASH * FIELD_HEIGHT) && flg != ((CHR_FLASH+1) * FIELD_HEIGHT) && ! is_flash_line() /*&& ! game_state.scroll_offset*/ )
  {
...
add_score( int line )
{
  if( ship.status != SHIP_BONUS )
    ...
    {
      level[ game_state.mode ].score += (FIELD_WIDTH - line) *  BASE_SCORE * BONUS_RATE;
    }
  update_scores();
}
...
set_aim( void )
{
  ...
  for( i = 0; i < FIELD_WIDTH; i++ )
    {
      if( meteo_field[ y * FIELD_WIDTH + i ] ) break;
    }
      ...
      if( !( RND( 0, STAR_RND ) ) )
        {
          map[i] = RND( 0, 3 );
          // 点滅ウェイト
          blink_star_interval[i] = DEF_STAR_INTERVAL + RND( 0, MAX_STAR_INTERVAL ) *4;
          blink_star_interval_work[i] = blink_star_interval[i];
        }
      else
        {
