...
const WaveListType MusicList[] = {
	{ m0_bin, SAMPLING_M0, 0 }, //データアドレス, データ長, 優先度
	{ m1_bin, SAMPLING_M1, 0 }, 
	{ m2_bin, SAMPLING_M2, 0 }, 
	{ m3_bin, SAMPLING_M3, 0 },
};



const WaveListType SoundList[] = {
	{ s0_bin, SAMPLING_E0, 50 }, //データアドレス, データ長, 優先度
	{ s1_bin, SAMPLING_E1, 40 }, 
	{ s2_bin, SAMPLING_E2, 10 }, 
	{ s3_bin, SAMPLING_E3, 20 }, 
	{ s4_bin, SAMPLING_E4, 30 }, 
};
...
// 楽曲の再生  引数= 曲番号、ループ
//*************************************************************
void PlayMusic(int _no,int loop)
{
	if( _no >= MAX_MUSIC ) return;
...
// 効果音の再生  引数=効果音番号
//*************************************************************
void PlaySound( int _no )
{
	if( _no >= MAX_SOUND ) return;
	
	// 優先度
	if( DSeffect.ds_Playing == 1 && SoundList[ DSeffect.ds_snd_no ].priority < SoundList[ _no ].priority ) return;
